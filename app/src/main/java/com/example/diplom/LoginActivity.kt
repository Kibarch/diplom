    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        loginBinding.buttonVoiti.setOnClickListener{
            ...
            db.getDao().getAllItem().asLiveData().observe(this){list->
                ...
                list.forEach{
                    val text = "flag: $flag Id: ${it.id} GlobalId: ${Global().getUserId()} emailtext:${loginBinding.textFieldLoginEmail.text} passwordText: ${loginBinding.textFieldLoginPassword.text} Email:${it.email} Passw:${it.password}\n"
                    ...
                    {
                        flag = 1
                        it.id?.let { it1 -> Global().setUserId(it1) }
                        loginBinding.textView3.append("currentID: ${it.id} GlobalId: ${Global().getUserId()} flag: $flag")
                    }
